...
class WalletController extends Controller
{
    ...
    public function walletDetails(Request $request, $id)
    {
        ...
        $primaryDefaultWallet = get_primary_wallet((int) Auth::id(), DEFAULT_COIN_TYPE);
        $primaryDefaultWalletId = (int) ($primaryDefaultWallet->id ?? 0);
    ...
    public function walletConnectGasTopup(Request $request)
    {
        try {
            ...
            $gasless = app(GaslessSponsorshipService::class);
            $result = $gasless->sponsor((int) Auth::id(), $walletAddress, 'transfer');

            if (!$result['success']) {
                return response()->json([
                    'success' => false,
                    'topup_sent' => false,
                    'message' => $result['message'] ?? __('Unable to sponsor gas right now.'),
                    'data' => $result['data'] ?? [],
                ], 422);
            }
            ...
            return response()->json([
                'success' => true,
                'topup_sent' => true,
                'user_tx_hash' => $result['data']['tx_hash'] ?? null,
                'user_bnb' => $result['data']['amount_native'] ?? null,
